...
export function PlayerToken({ avatar, label, tone, isReady }: { avatar: string; label: string; tone: GameSlot | "pending"; isReady?: boolean }) {
  return (
...
export function AvatarPicker({ selected, onPick }: { selected: string; onPick: (avatar: string) => void }) {
  return (
...
export function OutcomeBanner({ eventLog, playerSlot }: { eventLog: GameSnapshot["eventLog"]; playerSlot?: GameSlot }) {
  const t = useT();
...
export function RevengeGauge({ combatant, align }: { combatant: GameCombatant; align: "left" | "right" }) {
