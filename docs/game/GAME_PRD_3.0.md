...
- <a href="#section-updated-features-of-mvp">3. Features Of MVP</a>
   ...
   - <a href="#feature-visual-presentation-features">B) Visual Presentation Features</a>
      - <a href="#feature-avatar-and-cpu-character-art">i. Profile Picture / Player Avatar And CPU Character Art</a>
...
- <a href="#section-companion-documents">5. Companion Documents</a>

<a id="section-introduction"></a>

## 1. Introduction

PRD 3.0 documents and stabilizes the MVP features discussed in the Project Meeting on May 28.

This document is based on the work completed in PRD 1.0 and PRD 2.0, but PRD 3.0 is trimmed to focus on the current MVP implementation source of truth.

The purpose of PRD 3.0 is to define what should be implemented for MVP.
...
PRD 3.0 MVP is a fast competitive mental arithmetic duel where players feel pressure, mastery, comeback tension, and rivalry.
...
Use this section to define the confirmed MVP feature set for PRD 3.0.
...
- if a combatant reaches `0 HP`, the fight round ends immediately by KO.
- if one combatant reaches `0 HP`, the other combatant wins that fight round.
- if both combatants reach `0 HP` from the same resolved exchange, the fight round is tied.
...
- a fight round can end early by KO when a combatant reaches `0 HP`.
- if no KO occurs, the fight-round winner is determined by higher remaining HP at the end of the 60-second fight round.
...
The shop is not part of PRD 3.0 MVP.
...
There is no public-facing game lobby in PRD 3.0 MVP.
...
#### i. Profile Picture / Player Avatar And CPU Character Art

MVP player identity supports two image options:

- player-uploaded profile picture.
- premade player avatar selection.

Player identity image rules:

- players may upload a profile picture from their device.
- players may choose a premade player avatar if they do not want to upload a picture.
- supported image formats include `jpg`, `jpeg`, `png`, and `webp`.
- uploaded profile pictures are validated before storage.
- premade player avatars are stored as built-in frontend/game assets, not user-uploaded media.
- the selected identity image is displayed on profile, friend, community, leaderboard, VS, and active match surfaces.
- active match may animate the selected identity image for presentation, including HP-based heartbeat scaling.
- player identity images do not need separate smiling, focused, injured, win, or lose state assets for MVP.
...
- each player side shows the combatant identity image or CPU identity, HP bar, DEFEND button, attack status bar, and state status bar.
...
- almost immediately after the attack animation begins, show the opponent identity image or CPU identity hit animation.
...
- stored as player identity media.

Premade player avatar assets should be:

- stored as built-in frontend/game assets.
- selectable from the Profile page.
- usable anywhere a player identity image is displayed.
- visually distinct enough for friend, leaderboard, VS, and active match surfaces.
...
- shows each friend's selected identity image.
...
- search results show selected identity image, player name, and Aura Points.
...
- incoming requests show requester selected identity image, player name, and Aura Points.
...
- shows message sender selected identity image.
...
- friend-only results chat is not part of PRD 3.0 MVP.
...
- choose or replace a premade player avatar.
...
Profile identity image rules:

- the player's MVP identity image can be either a user-uploaded profile picture or a selected premade player avatar.
- if the player uploads a profile picture, that uploaded picture becomes the selected identity image.
- if the player chooses a premade avatar, that premade avatar becomes the selected identity image.
- the selected identity image is shown on profile, friend, community, leaderboard, VS, and active match surfaces where player identity is displayed.
- active match presentation may animate the selected identity image, but the selected image remains the identity source.

Active Match identity image heartbeat:

- player identity images on the active match screen scale up and down to simulate a heartbeat.
...
- public profile information includes username, selected identity image, Aura Points where relevant, and online/offline state where relevant.
...
MVP architecture uses the selected tech stack across four Dockerized runtime services.

| Service | Primary Responsibility |
| --- | --- |
| Nginx entrypoint / reverse proxy | Routes frontend, REST API, and WebSocket / Socket.IO upgrade traffic. |
| React + Next.js frontend | Owns app pages, routing, React page UI, REST and Socket.IO/WebSocket clients, localization resources, and the browser-loaded Active Match host. |
| Node.js + Express backend / modular monolith | Owns auth, profile, friends, stats, matchmaking, live match runtime, match summary persistence, question generation, CPU behavior, REST APIs, Socket.IO realtime handling, and server runtime memory. |
| PostgreSQL database service | Runs PostgreSQL and persists `users`, `player_profiles`, `player_friendships`, `pvp_matches`, and `player_cpu_progression`. |
...
- backend runtime memory owns temporary queues, rooms, ready/reconnect timers, active match state, WebSocket / Socket.IO sessions, and the latest-50 Community Chat buffer.
...
- REST is used for stable page data; WebSocket + Socket.IO realtime transport is used for queue, ready, active match, reconnect, rematch, and Community Chat flows.

Detailed implementation rules live in [GAME_PRD_3.0_IMPLEMENTATION.md](GAME_PRD_3.0_IMPLEMENTATION.md).
...
<a id="section-companion-documents"></a>

## 5. Companion Documents

Detailed implementation contract: [GAME_PRD_3.0_IMPLEMENTATION.md](GAME_PRD_3.0_IMPLEMENTATION.md)

Detailed page inventory: [GAME_PRD_3.0_PAGES.md](GAME_PRD_3.0_PAGES.md)

Detailed page testing checkpoints: [GAME_PRD_3.0_TEST_PLAN.md](GAME_PRD_3.0_TEST_PLAN.md)

Architecture visual reference: [GAME_PRD_3.0_ARCHITECTURE_VISUAL.html](GAME_PRD_3.0_ARCHITECTURE_VISUAL.html)
